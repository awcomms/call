...
export const POST: RequestHandler = async ({ request, params }) => {
	let id
	try {
		id = await request.text();
		const local_input = await get<V>(client, PREFIX.concat(params.id), ['$.input']);
		const remote_input = await get<V>(client, PREFIX.concat(id), ['$.input']);
		return text(
			await openai
		...
		throw handle_server_error(`POST /users/${id ? id : '<unknown_id>'}/similarity`, e);
	}
};
